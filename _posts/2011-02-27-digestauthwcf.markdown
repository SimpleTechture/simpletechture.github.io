...
This is a follow-up on [my earlier article]({% post_url 2011-02-28-basicauthwcf %}) that described how to use BASIC Authentication with a WCF REST Service. The disadvantage of [that solution]({% post_url 2011-02-28-basicauthwcf %}) was that you need a https tunnel to really secure the username password verification process. Although possible, this is not always a feasible situation, for example, if you don't want to invest in certificates or loose performance when using a https tunnel.
...
{% highlight ruby %}
PrivateHash = MD5Hash( TimeStamp : IP Address : Private key){% endhighlight %}
...
##Retrieving and Storing User Credentials</a></h3>
